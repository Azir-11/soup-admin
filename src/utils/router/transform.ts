...
import { notFound } from "@/router/routes/constant";
...
export function transformAuthRouteToVueRoute(item: AuthRoute.Route) {
  ...
  // 外链路由
  if (hasHref(item)) {
    Object.assign(itemRoute, { component: notFound });
  }
...
/** 将路由路径转换成路由名字 */
export const transformRoutePathToRouteName = <K extends AuthRoute.RoutePath>(path: K) => {
  if (path === "/") return "root";
